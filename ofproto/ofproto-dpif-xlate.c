...
/* Emit-sample-related arguments for compose_sample_action. */
struct sample_emit_args {
    uint32_t group_id;                  /* Group to be used in emit_sample. */
    struct ofpbuf cookie;               /* Cookie to be used in emit_sample. */
};

/* Upcall-related arguments for compose_sample_action. */
                      ...
                      const struct compose_sample_args *args)
{
    ...
    ovs_assert(args->upcall || args->emit);
    ...
    bool is_sample = (args->probability < UINT32_MAX ||
                      (args->upcall && meter_id != UINT32_MAX));
    ...
    if (args->upcall) {
        ...
        int res = odp_put_userspace_action(pid, &args->upcall->cookie,
                                           sizeof args->upcall->cookie,
                                           args->upcall->tunnel_out_port,
                                           args->upcall->include_actions,
                                           ctx->odp_actions, &cookie_offset);
        ovs_assert(res == 0);
    }

    if (args->last && ovs_explicit_drop_action_supported(ctx->xbridge->ofproto)) {
...
compose_sflow_action(struct xlate_ctx *ctx)
{
    ...
    struct sample_userspace_args upcall;

    if (!sflow || ctx->xin->flow.in_port.ofp_port == OFPP_NONE) {
        return 0;
    }

    memset(&upcall.cookie, 0, sizeof upcall.cookie);
    upcall.cookie.type = USER_ACTION_COOKIE_SFLOW;
    upcall.cookie.ofp_in_port = ctx->xin->flow.in_port.ofp_port;
    upcall.cookie.ofproto_uuid = ctx->xbridge->ofproto->uuid;
    upcall.tunnel_out_port = ODPP_NONE;
    upcall.include_actions = true;
...
compose_ipfix_action(struct xlate_ctx *ctx, odp_port_t output_odp_port)
{
    ...
    if (output_odp_port != ODPP_NONE) {
        ...
        if (dpif_ipfix_get_bridge_exporter_tunnel_sampling(ipfix) &&
           ...
           upcall.tunnel_out_port = output_odp_port;
        }
    }

    memset(&upcall.cookie, 0, sizeof upcall.cookie);
    upcall.cookie.type = USER_ACTION_COOKIE_IPFIX;
    upcall.cookie.ofp_in_port = ctx->xin->flow.in_port.ofp_port;
    upcall.cookie.ofproto_uuid = ctx->xbridge->ofproto->uuid;
    upcall.cookie.ipfix.output_odp_port = output_odp_port;
                        ...
                        struct sample_userspace_args *upcall)
{
    ...
    upcall->cookie.type = USER_ACTION_COOKIE_FLOW_SAMPLE;
    upcall->cookie.ofp_in_port = ctx->xin->flow.in_port.ofp_port;
    upcall->cookie.ofproto_uuid = ctx->xbridge->ofproto->uuid;
                    ...
                    bool last)
{
    ...
    struct sample_emit_args emit_sample;
    struct sample_userspace_args upcall;
    uint32_t group_id;
    ...
    if (!compose_args.upcall && !compose_args.emit) {
        return;
    }

    compose_sample_action(ctx, &compose_args);

    if (compose_args.emit) {
        ofpbuf_uninit(&compose_args.emit->cookie);
    }
}
