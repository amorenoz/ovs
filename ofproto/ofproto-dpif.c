...
bool
ovs_multicast_sample_supported(struct ofproto_dpif *ofproto)
{
    return ofproto->backer->rt_support.mcast_sample;
}
...
/* Tests whether 'backer''s datapath supports the OVS_USERSPACE_ATTR_MCAST.
 * attribute. */
static bool
check_mcast_sample(struct dpif_backer *backer)
{
    struct odputil_keybuf keybuf;
    struct ofpbuf actions;
    struct ofpbuf key;
    ...
    nl_msg_put_flag(&actions, OVS_USERSPACE_ATTR_MCAST);
    nl_msg_end_nested(&actions, offset);

    supported = dpif_probe_feature(backer->dpif, "mcast_sample", &key,
                                   &actions, NULL);
    ofpbuf_uninit(&actions);
...
check_support(struct dpif_backer *backer)
{
    ...
    backer->rt_support.mcast_sample = check_mcast_sample(backer);
...
get_datapath_cap(const char *datapath_type, struct smap *cap)
{
    ...
    smap_add(cap, "mcast_sample", s.mcast_sample? "true" : "false");
