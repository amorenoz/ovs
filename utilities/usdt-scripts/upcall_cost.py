...
#
class Event(object):
    def __init__(self, ts, pid, comm, cpu, event_type, queue_id, dpif_name="",
                 dp_port=None, pkt_len=None, pkt_frag_len=None,):
        self.ts = ts
        self.pid = pid
        self.comm = comm
        self.cpu = cpu
    ...
    def get_event_header_str():
        return "{:<28}  {:<21} {:<16} {:>8}  {:<3}  {:<18}  {}".format(
            "EVENT", "(IDs)", "COMM", "PID", "CPU", "TIME",
...
class OvsOperation(Event):
    def __init__(self, ts, pid, comm, cpu, event_type, queue_id, batch_ts,
                 batch_idx):
...
class DpUpcall(Event):
    def __init__(self, ts, pid, comm, cpu, dpif_name, port, pkt, pkt_len,
                 pkt_frag_len, upcall_id, queue_id):
        ...
        super(DpUpcall, self).__init__(ts, pid, comm, cpu, EventType.DP_UPCALL,
                                       queue_id, dpif_name, dp_port, pkt_len,
                                       pkt_frag_len)
    ...
    def handle_event(event):
        ...
        try:
            event = DpUpcall(event.ts, event.pid, event.comm.decode("utf-8"),
                             ...
                             pkt_data, event.pkt_size, event.pkt_frag_size,
                             event.upcall_id, event.queue_id)
        except LookupError:
            #
...
class RecvUpcall(Event):
    def __init__(self, ts, pid, comm, cpu, key, pkt, pkt_len, upcall_id,
                 queue_id, batch_ts, batch_idx):
        self.pkt = pkt
        self.key = key
        self.batch_ts = batch_ts
        self.batch_idx = batch_idx
        ...
        super(RecvUpcall, self).__init__(ts, pid, comm, cpu,
                                         EventType.RECV_UPCALL, queue_id, None,
                                         None, pkt_len)
    ...
    def handle_event(event):
        ...
        try:
            event = RecvUpcall(event.ts, event.pid, event.comm.decode("utf-8"),
                               event.cpu, bytes(event.key)[:key_len],
                               bytes(event.pkt)[:pkt_len], event.pkt_size,
                               event.upcall_id, event.queue_id, event.batch_ts,
                               event.batch_idx)
        except LookupError:
            return None
...
class DpUpcallQueue(Event):
    def __init__(self, ts, pid, comm, cpu, port, pkt_len, pkt_frag_len,
                 upcall_id, queue_id, res):
        self.res = res
        self.upcall_id = upcall_id
        self.nested = {}

        super(DpUpcallQueue, self).__init__(ts, pid, comm, cpu,
                                            EventType.DP_UPCALL_QUEUE,
                                            queue_id, "", port, pkt_len,
                                            pkt_frag_len)
    ...
    def append_event(self, event):
        if self.nested.get(event.event_type):
            raise ValueError(
                ...
                "This {}. Orig: {}".format(event.event_type, self, event,
                                           self.nested.get(event.event_type))
            )
        self.nested[event.event_type] = event

    def handle_event(event):
        event = DpUpcallQueue(event.ts, event.pid,
                              event.comm.decode("utf-8"), event.cpu,
                              event.dev_name.decode("utf-8"), event.pkt_size,
                              event.pkt_frag_size, event.upcall_id,
                              event.queue_id, event.res)
...
class EventTracer(object):
    ...
    def analyze(self, profile=False):
        ...
        with alive_bar(len(self.events),
                       title="- Matching DP_UPCALLs to RECV_UPCALLs",
                       spinner=None, disable=False) as bar:
        ...
        if self.received > 0:
            ...
            if self.stats["dup_upcalls"] > 0 or self.stats["dup_queue"] > 0:
                print("\nWARNING:"
                    " Some upcall events had the same identifiers!\n"
                    " Results might be inaccurate.\n")
                print("=> Upcall duplicates: {}\n"
        ...
        if options.show_groups or options.show_detail:
            ...
            for upcall in self.list_upcalls():
                ...
                for queue in upcall.queue_events:
                    print("DBG:   * {}{}{}{}{}".format(
                        "U",
                        "u" if EventType.RECV_UPCALL in queue.nested else "-",
                        "p" if EventType.OP_FLOW_PUT in queue.nested else "-",
                        "e" if EventType.OP_FLOW_EXECUTE in queue.nested
                        else "-",
                        "E" if EventType.OVS_PKT_EXEC in queue.nested
                        else "-"))
                    if detail:
                        try:
        ...
        show_key_value(self.per_thread,
                       description="Upcalls handled per thread")
        show_batch_histogram(self.per_batch.values(),
        ...
        with alive_bar(self.num_upcalls, title="- Calculating upcall cost",
                       spinner=None, disable=False,) as bar:
            for upcall in self.list_upcalls():
                for queue in upcall.queue_events:
                    nested = queue.nested
                    ...
                    if EventType.OP_FLOW_PUT in nested and \
                        EventType.OVS_PKT_EXEC in nested:
                    ...
                    if EventType.OP_FLOW_EXECUTE in nested and \
                        EventType.OVS_PKT_EXEC in nested:
...
def main():
    ...
    parser.add_argument("-g", "--show-groups",
                        help="Show processed upcall event groups in a"
                        " summarized format",
                        action="store_true", default=False)
    parser.add_argument("-d", "--show-detail",
                        help="Show processed upcall event groups in a"
                        " detailed format",
                        action="store_true", default=False)
    parser.add_argument("-k", "--show-packets",
                        help="Dump packet content of recv_upcall events"
                        "A non-zero value must set using --packet-size."
                        " Implies '-d'.",
                        action="store_true", default=False)
    parser.add_argument("-a", "--show-attributes",
                        help="Dump netlink attributes of recv_upcall events"
                        "A non-zero value must set using --flow-key-size."
                        " Implies '-d'.",
                        action="store_true", default=False)
    parser.add_argument("-f", "--flow-key-size",
                        help="Set maximum flow key size to capture, "
                        "default 0",
                        type=buffer_size_type, default=0, metavar="[0-2048]")
    ...
    parser.add_argument("-P", "--packet-size",
                        ...
                        "default 0", type=buffer_size_type, default=0,
                        metavar="[0-2048]")
    parser.add_argument("-p", "--pid", metavar="VSWITCHD_PID",
                        help="ovs-vswitch's PID", type=int, default=None)
    ...
    parser.add_argument("-s", "--stop",
                        ...
                        "events", type=int, default=0, metavar="EVENTS")
    parser.add_argument("-w", "--write-events", help="Write events to FILE",
    ...
    if not options.read_events and (
        ...
        print("ERROR: The --show-packets option requires a non-zero value on "
    ...
    if not options.read_events and (
        ...
        print("ERROR: The --show-attributes option requires a non-zero value "
