...
#
# Dependencies:
# This script depends on the scapy python library.
# Additionally, the script needs 'pahole' binary to be available and the kernel
# version to be at least v6.6.
#
try:
    from bcc import BPF
...
def get_skb_drop_reasons(pahole="pahole"):
    ...
    try:
        ovs_drop_reason = parse_enum(
            run_pahole(openvswitch, "ovs_drop_reason")
        )
    except PaholeError as err:
        print(f"ERROR: Cannot extract skb drop reasons {err}")
        sys.exit(-1)
...
class EventProcessor:
    ...
    def get_reason(self, event):
        ...
        return self.ovs_drop_reasons.get(
            event.reason, self.kfree_drop_reasons.get(event.reason, "UNKNOWN")
        )
    ...
    def receive(self, event):
        ...
        if self.options.write_events:
            ...
            self.writer.write_packet(
                pkt, sec=secs, caplen=cap_len, wirelen=pkt_len
            )
    ...
    def start(self):
        ...
        while 1:
            try:
                ...
                if (
                    self.options.stop != 0
                    and self.n_received >= self.options.stop
                ):
                    break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Capture packets dropped by "
    ...
    parser.add_argument(
        "-p",
        "--packet-size",
        help="Set maximum packet size to capture, " "default 256",
    ...
    parser.add_argument(
        "--print",
        help="Print events even if '--write-events' has been " "used",
        action="store_true",
    )
